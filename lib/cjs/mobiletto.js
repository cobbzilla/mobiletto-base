...
function mobiletto(driverPath, key, secret, opts, encryption) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let driver;
        if (register_1.ALL_DRIVERS[driverPath]) {
            driver = register_1.ALL_DRIVERS[driverPath];
        }
        else {
        ...
        if (!client.redisConfig.prefix) {
            client.redisConfig.prefix = util_1.REDIS_PREFIX;
        }
        ...
        if (!encryption) {
            ...
            return (0, functions_1.addUtilityFunctions)((0, functions_1.addCacheFunctions)(client), readOnly);
        }
        // Encryption is enabled
        // Wrap the client, override various methods to enable transparent encryption
        const encKey = (0, crypt_1.normalizeKey)(encryption.key);
        if (!encKey) {
            const message = `mobiletto(${driverPath}) invalid encryption key`;
            mobiletto_common_1.logger.error(message);
            throw new mobiletto_common_1.MobilettoError(message);
        }
        const iv = (0, crypt_1.normalizeIV)(encryption.iv, encKey);
        if (!iv) {
            const message = `mobiletto(${driverPath}) invalid encryption IV`;
            mobiletto_common_1.logger.error(message);
            throw new mobiletto_common_1.MobilettoError(message);
        }
        const dirLevels = encryption.dirLevels || crypt_1.DEFAULT_DIR_LEVELS;
        const algo = encryption.algo || crypt_1.DEFAULT_CRYPT_ALGO;
        const metaWorkers = encryption.metaWorkers || crypt_1.DEFAULT_META_WORKERS;
        const enc = {
            key: encKey,
            iv,
            algo,
        ...
        const outerClient = (0, functions_1.addCacheFunctions)(client);
        const _metadata = (client) => (path) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const chunks = [];
                yield client.read(metaPath(path), (0, util_1.reader)(chunks));
                metaObj = JSON.parse((0, crypt_1.decrypt)(Buffer.concat(chunks).toString(), enc));
            }
            catch (e) {
                metaObj = {};
            }
        ...
        const _singleMeta = (job) => __awaiter(this, void 0, void 0, function* () {
            ...
            return new Promise((resolve, reject) => {
                const cipherText = [];
                client
                    .read(dirent + "/" + (0, path_1.basename)(entry.name), (0, util_1.reader)(cipherText))
                    .then((bytesRead) => {
                    if (!bytesRead) {
                        ...
                        const plain = (0, crypt_1.decrypt)(cipherText.toString(), enc);
        ...
        const metaLoadQueue = () => {
            if (META_LOAD_QUEUE === null) {
                ...
                const port = client.redisConfig.port || parseInt(`${util_1.REDIS_PORT}`);
                META_LOAD_QUEUE = new bullmq_1.Queue(META_LOAD_QUEUE_NAME, {
                    connection: {
                        host: client.redisConfig.host || util_1.REDIS_HOST,
                        port,
                    },
                    prefix: client.redisConfig.prefix + "_" + META_LOAD_QUEUE_NAME,
                });
                const numWorkers = enc.metaWorkers || crypt_1.DEFAULT_META_WORKERS;
                for (let i = 0; i < numWorkers; i++) {
                    META_WORKERS.push(new bullmq_1.Worker(META_LOAD_QUEUE_NAME, _singleMeta));
                }
                const queueEvents = new bullmq_1.QueueEvents(META_LOAD_QUEUE_NAME);
        ...
        const encClient = {
            ...
            read: (path, callback, endCallback) => __awaiter(this, void 0, void 0, function* () {
                ...
                const cipher = (0, crypt_1.getDecipher)(enc);
            ...
            write: (path, readFunc) => __awaiter(this, void 0, void 0, function* () {
                ...
                while (true) {
                    ...
                    const direntGenerator = (0, util_1.stringGenerator)(p + enc.encPathPadding(), enc);
                ...
                const cipher = (0, crypt_1.getCipher)(enc);
                const realPath = encryptPath(path);
                ...
                if ((0, mobiletto_common_1.isReadable)(readFunc)) {
                    ...
                    yield client.write(realPath, (0, util_1.newCryptGenerator)(readFunc, generatorBytes, cipher));
                }
                // write metadata
                const meta = { name: path, size: generatorBytes, type: mobiletto_common_1.M_FILE };
                yield client.write(metaPath(path), (0, util_1.stringGenerator)(JSON.stringify(meta), enc)());
        ...
        return (0, functions_1.addUtilityFunctions)((0, functions_1.addCacheFunctions)(encClient), readOnly);
    });
}
