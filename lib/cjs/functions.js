...
const UTILITY_FUNCTIONS = {
    ...
    readFile: (client) => (path) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const chunks = [];
        yield client.read(path, (0, util_1.reader)(chunks));
    ...
    mirror: (client) => (source, clientPath = "", sourcePath = "") => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const visitor = (obj) => __awaiter(void 0, void 0, void 0, function* () {
            if (obj.type && obj.type === mobiletto_common_1.M_FILE) {
                ...
                const tempPath = `${util_1.MOBILETTO_TMP}/mobiletto_${(0, shasum_1.default)(JSON.stringify(obj))}.${randomstring_1.default.generate(10)}`;
...
const CACHE_FUNCTIONS = {
    redis: (client) => () => {
        ...
        if (!enabled) {
            ...
            client.cache = cache_1.DISABLED_CACHE;
            return client.cache;
        }
        const host = redisConfig.host || util_1.REDIS_HOST;
        const port = redisConfig.port || parseInt(`${util_1.REDIS_PORT}`);
        const prefix = redisConfig.prefix || util_1.REDIS_PREFIX;
        if (!client.id) {
            mobiletto_common_1.logger.warn(`redis: all nameless connections will share one cache`);
            client.cache = (0, redis_1.getRedis)("~nameless~", host, port, prefix);
        }
        else {
            client.cache = (0, redis_1.getRedis)(client.id, host, port, prefix);
        }
        return client.cache;
    },
    scopedCache: (client) => (cacheName, size = 100) => {
        const cache = client.redis();
        return cache instanceof redis_1.MobilettoCache ? cache.scopedCache(cacheName, size) : new cache_1.AwaitableLRU(size);
    },
