...
const shutdownMobiletto = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    mobiletto_1.ALL_META_WORKERS.forEach((w) => workerClosePromises.push(new Promise((resolve, reject) => {
        w.close(true)
            .then(() => resolve)
            .catch((e) => {
            mobiletto_common_1.logger.info(`shutdownMobiletto: error closing queue worker: ${e}`);
            reject(e);
        });
    })));
    try {
        ...
        mobiletto_common_1.logger.warn(`shutdownMobiletto: error cleaning up queue workers: ${e}`);
    }
    try {
        yield (0, redis_js_1.teardown)();
    }
    catch (e) {
        mobiletto_common_1.logger.warn(`shutdownMobiletto: error cleaning up cache: ${e}`);
    }
});
